...
public class SongFragment extends BaseFragment implements OnPullZoomListener, OnListScrollListener, OnClickListener
{
	...
	private Handler handler = new Handler(new Callback()
	{
		...
		public boolean handleMessage(Message msg)
		{
			...
			else if(msg.what == ClientHandler.RECV_PLAY_ERROR)
			{
				...
				showSongErrorDialog(getActivity(), "鐐规挱澶辫触", playErrorCmd.getMediaName(), "璇ユ枃浠舵牸寮忎笉鏀寔锛岃閲嶈瘯鎴栬�呮悳绱㈣姝屾洸");
				//getPlayList(0);
			}
			...
			else if(msg.what == ClientHandler.MINA_CONNECT_ERROR)
			{
				...
				{
					showHintRefreshView();
					//showToast("鎻愮ず锛氭湇鍔″櫒鏂紑杩炴帴");
				}
				else if(type == MinaErrorType.EXCEPTION)
					showToast("鎿嶄綔澶辫触锛屾湇鍔″櫒寮傚父");
	...
	*/
	...
	{
		isReconnect = false;
		tvHint.setText("鏈繛鎺ifi缃戠粶锛岃鍏堣繛鎺�");
		tvHint.setTextColor(getResources().getColor(R.color.text_black_on));
		tvHint.setVisibility(View.VISIBLE);
		tvDianbo.setVisibility(View.VISIBLE);
		tvDeviceSelect.setVisibility(View.GONE);
		tvDianbo.setText("鎵弿璁惧");
		tvDianbo.setTextColor(getResources().getColor(R.color.text_black_on));
	}
	...
	private void updatePlayInfor(SongInfor cmdInfor)
	{
		...
		{
			tvMusicName.setText("姝ｅ湪鎾斁锛�" + curSongInfor.getMediaName());
			tvMusicNameTitle.setText("姝ｅ湪鎾斁锛�" + curSongInfor.getMediaName());
		...
		{
			tvMusicName.setText("绛夊緟鐐规挱...");
			tvMusicNameTitle.setText("绛夊緟鐐规挱...");
			ivStatus.setImageDrawable(getResources().getDrawable(R.drawable.icon_play_status_wait));
			
			//marqueeTextView.setText("浜轰汉鐐规瓕鍏祴鐗堟寮忎笂绾匡紝涓嬭浇灏遍��9999閲戝竵锛屾杩庡ぇ瀹朵綋楠岋紒");
	...
	private void refreshStart()
	{
		...
		tvRefresh.setVisibility(View.VISIBLE);
		ivRefresh.clearAnimation();
		tvRefresh.setText("鏉惧紑鍒锋柊鏁版嵁");
	}
	private void refreshIng()
	{
		ivRefresh.setVisibility(View.VISIBLE);
		tvRefresh.setVisibility(View.VISIBLE);
		tvRefresh.setText("鏁版嵁鍔犺浇涓�...");
	...
	private void showFooterNoMore()
	{
		if(mDiangeManger.isDeviceFind(false))
		{
			...
			tvFooterView.setText("娌℃湁鏇村鏁版嵁浜�");
		}
	}
	private void showFooterLoadFail()
	{
		...
		{
			pbFooterView.setVisibility(View.INVISIBLE);
			tvFooterView.setText("鍔犺浇鏁版嵁澶辫触锛岃閲嶈瘯");
		}
	}
	...
	{
		showProgressDialog(getActivity(), "姝ｅ湪澶勭悊...");
	...
	*/
	...
	{
		// TODO Auto-generated method stub
		//涓嬫媺鍒扮┖闂叉槸锛屼笖鏈�鍚庝竴涓猧tem鐨勬暟绛変簬鏁版嵁鐨勬�绘暟鏃讹紝杩涜鏇存柊
	...
	*/
	...
	{
		// TODO Auto-generated method stub
        lastItem = firstVisibleItem + visibleItemCount - 2;  //鍑�2鏄洜涓哄鍔犱簡header鍜宖ooter
	}
	/**
	*callbacks
	*/
