		...
		public void handleMessage(android.os.Message msg) 
		{
			...
			else if(msg.what == HttpMsg.GET_BIND_SPEAKERS_SUCCESS)
			{
				...
				if((tempList.size() == 0) && (pageNum == 1))
				{
					...
					dismiss();
					Toast.makeText(context, "您还没有添加店铺哦", 0).show();
				}
				else
				{
			...
			else if(msg.what == HttpMsg.GET_BIND_SPEAKERS_FAIL)
			{
				isRunning = false;
				onLoad(0);
				//showHint("鑾峰彇搴楅摵鍒楄〃澶辫触");
				Toast.makeText(context, "获取店铺信息失败", 0).show();
				//showToast("鑾峰彇鏁版嵁澶辫触锛岃涓嬫媺鍒锋柊");
			}
			else if(msg.what == HttpMsg.PUSH_MUSIC_START)
			{
				showProgressDialog(context, "正在插播...");
			}
			else if(msg.what == HttpMsg.PUSH_MUSIC_SUCCESS)
			{
				new Handler().postDelayed(new Runnable() {
					...
					public void run() {
						...
						Toast.makeText(context, "插播成功", 0).show();
						dismissDialog();
						dismiss();
					}
	...
	*/
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int pos, long arg3)
	{
		...
		if(devInfor.isOnline())
			...
			Toast.makeText(context, "设备不在线，无法插播", 0).show();
	}
