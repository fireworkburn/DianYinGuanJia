...
public class MyMusicHeaderView extends RelativeLayout
{
	...
	private Handler handler = new Handler()
	{
		public void handleMessage(android.os.Message msg) 
		{
			...
			{
				/*Toast.makeText(context, "鍒涘缓鎴愬姛", 0).show();
				((MusicFragment)baseActivity).getMusicAlbums();*/
			}
			else if(msg.what == HttpMsg.CREATE_ALBUM_FAIL)
			{
				//Toast.makeText(baseActivity.getActivity(), "鍒涘缓澶辫触", 0).show();
			}
		};
	};
	...
	*/
	...
	{
		tvHintOne.setText(context.getResources().getString(R.string.album_create) + "("+ infor.getCreateAlbumCount() + ")");
		tvHintTwo.setText(context.getResources().getString(R.string.album_collect) + "("+ infor.getCollectAlbumCount() + ")");
	...
	private MusicAlbumInfor getUploadMusicAlbum()
	{
		MusicAlbumInfor infor = new MusicAlbumInfor();
		infor.setAlbumId("0");
		infor.setAlbumName(context.getResources().getString(R.string.album_update));
		infor.setDescription(context.getResources().getString(R.string.album_update_desc));
		return infor;
	}
	...
	private void showEditDialog(final String infor)
	{
		...
		playDialog.setOnClickListener(new android.view.View.OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				{
					Toast.makeText(baseActivity.getActivity(), "璇疯緭鍏ュ悕绉�", 0).show();
					return;
				}
				playDialog.dismiss();
